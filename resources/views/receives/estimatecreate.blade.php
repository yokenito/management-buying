...
@section('content')
    <main>
        <h2 class="section-ttl">納品書の作成</h2>
        @if ($errors->any())
            <div>
                <ul>
        ...
        <form action="{{route('receives.estimatestore', $estimate)}}" method="post" class="w-75">
            @csrf
            <div>
            ...
            <div class="mt-4">
                ...
                <div class="d-flex">
                    ...
                    <input type="hidden" name="client_id" class="form-control w-50" id="set-client-id" value="{{old('client_id',$estimate->client_id)}}" readonly>
                    <div class="my-parts" data-bs-toggle="modal" data-bs-target="#searchClientModal"><span></span></div>
                </div>
            </div>
            <div class="mt-4">
                <label for="client_id" class="form-label">顧客名<span class="form-note">必須</span></label>
                <input type="text" name="client_id" class="form-control" value="{{old('client_id',$estimate->client->id)}}">
            </div>
            <div class="mt-4">
            ...
            <div class="mt-4">
                ...
                <textarea name="receive_requirement" class="form-control">{{old('receive_requirement', $estimate->estimate_requirement)}}</textarea>
            </div>
            <div class="mt-4">
